...
class UCB(Algorithm):
    def __init__(self, num_arms, horizon):
        ...
        self.counts = np.ones(self.num_arms)
...
class Thompson_Sampling(Algorithm):
    def __init__(self, num_arms, horizon):
        ...
        self.failure = np.zeros(self.num_arms)

        # END EDITING HERE
    
    def give_pull(self):
        # START EDITING HERE
        return np.argmax([np.random.default_rng().beta(self.success[i]+1, self.failure[i]+1) for i in range(self.num_arms)])
        # END EDITING HERE
