...
task1_avg = [191.2, 134.1, 176.7, 152.5,
             189.6, 138.5, 162.7, 169.4, 145.5, 162.1]
task2_avg = [239.3, 224.0, 257.0, 218.6,
             218.4, 238.4, 298.4, 226.0, 183.7, 225.9]
flag_ok = 0
...
class VerifyOutputPlanner:
    def __init__(self, task, random_seeds):
        ...
        for seed in random_seeds:
            ...
            print("\n\n", "-"*100)
            cmd_planner = "python", "run_simulator.py", "--task", task, "--random_seed", str(
                seed), "--frames_per_sec", str(150)

            print('Test case', str(counter), 'seed =',
                  str(seed), ":\t", " ".join(cmd_planner))

            cmd_output = subprocess.check_output(
                cmd_planner, universal_newlines=True)
            roadFlag, time_success = self.verifyOutput(
                cmd_output, task, counter-1)
    ...
    def verifyOutput(self, cmd_output, task, counter):
        ...
        if not len(est) == 10:
            mistakeFlag = True
            print("\n", "*"*10, "Mistake: Exact number of lines in the standard output should be",
                  10, "but has", len(est), "*"*10)
