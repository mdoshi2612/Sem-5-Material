...
class Task1():
    ...
    def __init__(self):
        ...
        """
        super().__init__()
        self.required_angle_achieved = False
    ...
    def next_action(self, state):
        ...
        required_angle = np.arctan2(-curr_y, 350-curr_x) * 180 / np.pi
        if (required_angle < 0):
            required_angle += 360

        # if (abs(curr_angle - required_angle) > 4):
        #     action_steer = 2
        # else:
        #     action_steer = 1
        #     action_acc = 4

        if (not self.required_angle_achieved):
            if (abs(curr_angle - required_angle) > 4):
                action_steer = 2
            else:
                self.required_angle_achieved = True
                action_steer = 1

        if (self.required_angle_achieved):
            action_steer = 1
            action_acc = 4
    ...
    def controller_task1(self, config_filepath=None, render_mode=False):
        ...
        simulator = DrivingEnv('T1', render_mode=render_mode,
        ...
        for e in range(NUM_EPISODES):
            ...
            for t in range(TIMESTEPS):
                ...
                state, reward, terminate, reached_road, info_dict = simulator._step(
                    action)
                fpsClock.tick(FPS)
...
class Task2():
    ...
    def controller_task2(self, config_filepath=None, render_mode=False):
        ...
        for e in range(NUM_EPISODES):
            self.phase_1 = False
            self.phase_2 = False
            self.required_angle_1_achieved = False
            self.required_angle_2_achieved = False
            self.required_angle_3_achieved = False
            self.line = -1
            ...
            simulator = DrivingEnv('T2', eligible_list, render_mode=render_mode,
                                   config_filepath=config_filepath, ran_cen_list=ran_cen_list)
...
if __name__ == '__main__':
    ...
    # Keep this as the default while running your simulation to visualize results
    parser.add_argument("-f", "--frames_per_sec",
                        help="fps", type=int, default=30)
    ...
    if task == 'T1':
        ...
        agent = Task1()
        agent.controller_task1(
            config_filepath=config_filepath, render_mode=render_mode)

    else:

        agent = Task2()
        agent.controller_task2(
            config_filepath=config_filepath, render_mode=render_mode)
